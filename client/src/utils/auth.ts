...
class AuthService {
  ...
  getUser() {
    ...
    if (token) {
      const decoded = this.getProfile();  // Decode the token to get user info
      return decoded?.data;  // Assuming your token has user data in the `data` field
    }
    return null;  // Return null if there is no token or user data
  }
  getProfile() {
